//
//  ZQCourseSectionController.swift
//  ZQOpenTools
//
...
/// 课程分区控制器
class ZQCourseSectionController: ListBindingSectionController<ListDiffable> {
    
    private var vm: ZQDiffableBox<ZQIGListKitCourseModel>!
    
    private lazy var contentHeight:CGFloat = 0
    
    override func didUpdate(to object: Any) {
        guard let aModel = object as? ZQDiffableBox<ZQIGListKitCourseModel> else { return }
        vm = aModel
...
extension ZQCourseSectionController : ListBindingSectionControllerDataSource, ListBindingSectionControllerSelectionDelegate, ListSupplementaryViewSource {
    ...
    func sectionController(_ sectionController: ListBindingSectionController<ListDiffable>, cellForViewModel viewModel: Any, at index: Int) -> UICollectionViewCell & ListBindable {
        guard let cell = collectionContext?.dequeueReusableCell(of: ZQCourseCollectionViewCell.self, for: self, at: index) as? ZQCourseCollectionViewCell else { return ZQCourseCollectionViewCell() }
        return cell
    }
